...
std::map<std::string, std::string> blogs;

// function to view a blog
void view_blog(){
    // iterating through all the blogs and printing them
    for(auto i = blogs.begin(); i != blogs.end(); i++){
        std::cout << "Blog Title: " << i->first << std::endl;
        std::cout << "Description: " << i->second << std::endl;
    }
}
...
void edit_blog(){
    ...
    // asking user for the blog name
    std::cout << "Enter your blog name: ";
    std::getline(std::cin, name);

    // if the given name doesnt exist, return
    if(blogs.find(name) == blogs.end()){
        std::cout << "Entered blog doesnot exist.";
        return;
    }

    // asking for new description
    std::cout << "Enter your blog description: ";
    std::getline(std::cin, desc);

    // updating blog
    std::cout << "Successfully updated blog.";
    blogs[name] = desc;
}
...
void publish_blog(){
    ...
    // creating a new blog
    std::cout << "Successfully published a blog.";
    blogs[name] = desc;
}
...
int main(int argc, char** argv){
    ...
    menu.set_heading("Blog Editor");
    menu.set_colors("31", "32");
    menu.add_field("View Blogs", view_blog);
    menu.add_field("Edit Blog", edit_blog);
    menu.add_field("Publish a Blog", publish_blog);
    menu.set_max_len(40);
